...
// Les traqueurs créent une empreinte en analysant comment le matériel
// audio traite un signal. Le résultat est stocké dans l'objet Buffer.

if (typeof window.AudioBuffer !== 'undefined' && window.AudioBuffer.prototype && typeof window.AudioBuffer.prototype.getChannelData === 'function') {
    ...
    window.AudioBuffer.prototype.getChannelData = function(...args) {
        try {
            ...
            // Appeler la fonction originale pour obtenir les vraies données
            const data = originalGetChannelData.apply(this, args);

            // Appliquer un bruit infime au premier élément du tableau de données.
            // C'est suffisant pour changer le hachage, mais n'affecte pas l'écoute.
